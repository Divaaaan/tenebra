...
//
// Windows only, and the build tag is load-bearing: these bytes are a Windows
// packet filter, so on macOS and Linux they would be a megabyte and a half of
// payload that can never execute plus four megabytes unpacked into a config
// directory for nothing (see embedded_other.go).
//
