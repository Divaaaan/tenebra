...
func (r *Runner) ConnectionsJSON(ctx context.Context) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("linux: clash connections: %w", err)
	}
	setClashAuth(req, r.clashAuth())
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("linux: clash connections: status %s", resp.Status)
	}
	body, err := io.ReadAll(io.LimitReader(resp.Body, maxConnectionsBody))
	if err != nil {
		return nil, fmt.Errorf("linux: clash connections: read: %w", err)
	}
	return body, nil
}
