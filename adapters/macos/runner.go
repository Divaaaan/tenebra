...
func (r *Runner) Select(ctx context.Context, group, tag string) error {
	...
	if err != nil {
		return fmt.Errorf("macos: clash select body: %w", err)
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPut, selectURL(port, group), bytes.NewReader(body))
	if err != nil {
		return fmt.Errorf("macos: clash select request: %w", err)
	}
	...
	resp, err := client.Do(req)
	if err != nil {
		return fmt.Errorf("macos: clash select: %w", err)
	}
	defer resp.Body.Close()
	rb, _ := io.ReadAll(io.LimitReader(resp.Body, 1<<16))
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		return fmt.Errorf("macos: clash select: status %s: %s", resp.Status, trimBody(rb))
	}
	return nil
}
