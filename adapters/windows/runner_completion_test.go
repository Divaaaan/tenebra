...
const runnerHelperModeEnv = "TENEBRA_WINDOWS_RUNNER_HELPER_MODE"
...
func TestRunnerDoneIncludesFinalStdoutAndStderr(t *testing.T) {
	...
	r := New()
	r.binOverride = runnerHelperBinary(t)
...
func runnerHelperBinary(t *testing.T) string {
	t.Helper()
	...
	dir := t.TempDir()
	dst := filepath.Join(dir, "runner-helper.exe")
	if err := copyFile(dst, exe); err != nil {
		t.Fatalf("copy helper binary: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "wintun.dll"), nil, 0o600); err != nil {
		t.Fatalf("write test wintun.dll: %v", err)
	}
	return dst
}
